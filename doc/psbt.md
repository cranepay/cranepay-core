# PSBT Howto for Cranepay Core

Since Cranepay Core 2.0.0, an RPC interface exists for Partially Signed Cranepay
...
PSBT is an interchange format for Cranepay transactions that are not fully signed
...
Overall, the construction of a fully signed Cranepay transaction goes through the
following steps:
...
- An **Extractor** produces a valid Cranepay transaction (in network format)
...
#### Multisig with multiple Cranepay Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Cranepay Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `cranepacranepayis used.
...
Later, when *V* CRP has been received on *Amulti*, and Bob and Carol want to
