...
The headless daemon `cranepayd` has the JSON-RPC API enabled by default, the GUI
`cranepay-qt` has it disabled by default. This can be changed with the `-server`
