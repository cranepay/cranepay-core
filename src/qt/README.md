...
Represents the main window of the Cranepay UI.
...
ClientModel is used by the main application `cranepaygui` and several models like `peertablemodel`.
...
Handles URI based application switching (e.g. when following a cranepay:... link from a browser).
...
* `bitcoinunits.(h/cpp)`: CRP / mCRP / etc handling
* `callback.h`
...
4. Enter "cranepay-qt" as project name, enter src/qt as location
